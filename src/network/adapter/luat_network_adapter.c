...
static int32_t network_default_socket_callback(void *data, void *param)
{
	...
	if (event->ID > EV_NW_TIMEOUT)
	{
		if (ctrl && ((event->ID == EV_NW_DNS_RESULT) || (ctrl->tag == cb_param->tag)))
		{
			...
			{
				DBG("socket event:0x%x(%s),wait:%d(%s)", event->ID, network_ctrl_event_id_string(event->ID), ctrl->wait_target_state, network_ctrl_wait_state_string(ctrl->wait_target_state));
				return 0;
			}
			if (ctrl->auto_mode)
			{
				DBG("before process socket %d,event:0x%x(%s),state:%d(%s),wait:%d(%s)", ctrl->socket_id, event->ID, network_ctrl_event_id_string(event->ID),
						ctrl->state, network_ctrl_state_string(ctrl->state),
						ctrl->wait_target_state, network_ctrl_wait_state_string(ctrl->wait_target_state));
				network_default_statemachine(ctrl, event, adapter);
				DBG("after process socket %d,state:%d(%s),wait:%d(%s)", ctrl->socket_id, ctrl->state, network_ctrl_state_string(ctrl->state),ctrl->wait_target_state, network_ctrl_wait_state_string(ctrl->wait_target_state));
			}
